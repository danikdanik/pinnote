...
- [Why PinNote](#why-pinnote)
- [Delivery methods](#delivery-methods)
  - [Method 1 — DevTools snippet](#method-1--devtools-snippet-chromium-based-browsers)
  - [Method 2 — Chrome extension](#method-2--chrome-extension)
- [Tags](#tags)
...
```
  You annotate  -->  notes.md  -->  Agent acts  -->  writes back  -->  You load  -->  repeat

  You:    Shift+Click elements, write notes, export notes.md
  Agent:  Reads notes + meta-prompt, applies changes, sets status fields
  You:    Click Load, pins recolor (green=applied, yellow=clarify, gray=skipped)
  Loop:   Clarify, add notes, re-export until every note is resolved
```
...
- **No extension install required.** The DevTools snippet works on any Chromium-based browser, any page (including `chrome://`), no store install. Lower friction than any hosted tool.
- **Human-in-the-loop by design.** The agent is instructed not to delete your notes or guess your intent. If something is unclear, it sets `needs-clarification` and asks. You stay in control.
...
### Method 1 — DevTools snippet (Chromium-based browsers)

Works on any page, including `chrome://` and `about:`. No installation needed, but DevTools must be open. Snippets are a Chromium DevTools feature (Chrome, Edge, Brave, etc.).
...
After running `npm run build`, open `examples/playground.html` in your browser. PinNote loads automatically. Shift+Click any element to try it.
